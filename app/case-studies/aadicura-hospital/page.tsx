  ...
  return (
    <main className="riot riot-longform" style={{ background: 'var(--r-black)', color: 'var(--r-cream)' }}>
      ...
      <section style={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'linear-gradient(100deg, rgba(12,12,12,.9) 0%, rgba(12,12,12,.62) 42%, rgba(12,12,12,.3) 72%, rgba(12,12,12,.6) 100%)' }} />
        ...
        <div style={{ position: 'absolute', left: 'clamp(20px,5vw,56px)', bottom: 32, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 9, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Scroll to begin</span>
        </div>
      </section>
      ...
      <section style={{ padding: 'clamp(80px,11vh,140px) clamp(20px,5vw,56px)', display: 'flex', flexWrap: 'wrap', gap: 'clamp(40px,6vw,80px)' }}>
        <Reveal style={{ flex: '1 1 420px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <span style={{ fontSize: 11, letterSpacing: '.18em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Six months · one embedded crew · nothing staged</span>
        </Reveal>
        <Reveal delay={120} style={{ flex: '1 1 320px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18, padding: 'clamp(24px,2.4vw,34px)', border: '1px solid rgba(243,240,221,.1)', borderRadius: 16, background: '#1B1B18' }}>
            {overviewMeta.map((m) => (
              <div key={m.label} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <span style={{ fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>{m.label}</span>
                <span style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--r-cream)' }}>{m.value}</span>
              </div>
            ))}
          </div>
        </Reveal>
      ...
      {/* What we created */}
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', background: '#191915', display: 'flex', flexDirection: 'column', gap: 'clamp(44px,6vh,72px)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.62)' }}>
      ...
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', display: 'flex', flexDirection: 'column', gap: 'clamp(28px,4vh,44px)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 24, border: '1px solid rgba(243,240,221,.12)', borderRadius: 14 }}>
            <span style={{ fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Before</span>
            {['Eight logo variants, no system', 'Departments buried four clicks deep', 'Announcement-only social feed', 'Licensed stock photography'].map((line) => (
              <span key={line} style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(243,240,221,.62)' }}>{line}</span>
            ))}
          </div>
          <div
            style={{
              ...
              borderRadius: 14,
              background: 'linear-gradient(180deg, rgba(237,230,76,.1), transparent)',
            }}
          >
            ...
            {['One mark, one typeface, one voice', 'Site opening on the people who staff it', 'Faces in sequence, three times a week', 'A 900-frame documentary library'].map((line) => (
              <span key={line} style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--r-cream)' }}>{line}</span>
            ))}
          </div>
        </div>
        <p style={{ margin: 0, maxWidth: '70ch', fontSize: 15, lineHeight: 1.75, color: 'rgba(243,240,221,.62)' }}>
          A complete transformation from a corporate healthcare brand into a human-first, documentary-driven identity.
        </p>
      </section>

      {/* Impact */}
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', background: '#191915', display: 'flex', flexDirection: 'column', gap: 'clamp(40px,5vh,60px)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <section style={{ padding: 'clamp(70px,10vh,120px) clamp(20px,5vw,56px)', display: 'flex', justifyContent: 'center' }}>
        <Reveal
          style={{
            ...
            textAlign: 'center',
            border: '1px solid rgba(243,240,221,.1)',
            borderRadius: 18,
            background: 'linear-gradient(180deg, #1B1B18, #141412)',
          }}
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, textAlign: 'left' }}>
              ...
              <span style={{ fontSize: 12, color: 'rgba(243,240,221,.62)' }}>Chief of Communications</span>
              <span style={{ fontSize: 12, color: 'rgba(243,240,221,.62)' }}>Aadicura Hospital</span>
            </div>
      ...
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', display: 'flex', flexDirection: 'column', gap: 'clamp(32px,4vh,48px)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 22 }}>
          {[
            ...
            >
              <div style={{ position: 'relative', aspectRatio: '16 / 10', overflow: 'hidden', borderRadius: 24, background: '#1B1B18', border: '1px solid rgba(243,240,221,.08)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14 }}>
          <Link href="/#case-studies" style={{ padding: '12px 24px', borderRadius: 999, border: '1px solid rgba(243,240,221,.18)', fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase' }}>
            ← Previous project
          </Link>
          <Link href="/#case-studies" style={{ padding: '12px 24px', borderRadius: 999, border: `1px solid ${ACCENT}`, background: 'rgba(237,230,76,.14)', fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase' }}>
            Next project →
          </Link>
        </div>
      </section>
