...
export default function AsterHealthPage() {
  return (
    <main className="riot riot-longform" style={{ background: 'var(--r-black)', color: 'var(--r-cream)' }}>
      <RiotNav />

      {/* Hero */}
      <section style={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        <MediaSlot placeholder="Cinematic hero film — slow push through the ward at dawn" />
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'linear-gradient(180deg, rgba(12,12,12,.62), transparent 30%, rgba(12,12,12,.92))' }} />
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 'clamp(18px,2.8vh,30px)', padding: 'clamp(80px,10vh,120px) clamp(20px,5vw,56px) clamp(60px,8vh,90px)' }}>
          ...
          <p style={{ margin: 0, maxWidth: '46ch', fontFamily: "'Instrument Serif', Georgia, serif", fontStyle: 'italic', fontSize: 'clamp(19px,1.9vw,28px)', lineHeight: 1.4, color: 'var(--r-cream)' }}>
        ...
        <div style={{ position: 'absolute', right: 'clamp(20px,5vw,56px)', bottom: 32, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 9, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Scroll</span>
          <span style={{ width: 1, height: 34, background: 'linear-gradient(180deg, rgba(243,240,221,.6), transparent)' }} />
        </div>
      </section>
      ...
      <section style={{ padding: 'clamp(80px,11vh,140px) clamp(20px,5vw,56px)', display: 'flex', flexWrap: 'wrap', gap: 'clamp(40px,6vw,80px)' }}>
        ...
        <Reveal delay={120} style={{ flex: '1 1 320px', display: 'flex', flexDirection: 'column', gap: 18, paddingTop: 20, borderTop: '1px solid rgba(243,240,221,.1)' }}>
          <span style={{ fontSize: 13, color: 'rgba(243,240,221,.62)' }}>
            {overviewRows.map((r) => `${r.label}: ${r.value}`).join(' · ')}
          </span>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Services</span>
            <span style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(243,240,221,.82)' }}>Brand strategy, Creative direction, Photography, Video production, Social media, Website</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Deliverables</span>
            <span style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(243,240,221,.82)' }}>Brand film, Social campaign, Photography library, Identity refresh, Motion assets</span>
          </div>
        </Reveal>
      </section>
      ...
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', display: 'flex', flexDirection: 'column', gap: 'clamp(28px,4vh,44px)' }}>
        ...
        <Reveal style={{ position: 'relative', aspectRatio: '21 / 9', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(243,240,221,.08)' }}>
          <MediaSlot placeholder="Cinematic still — empty corridor, 4am, available light" />
          <span style={{ position: 'absolute', bottom: 18, left: 18, padding: '8px 16px', borderRadius: 999, background: 'rgba(12,12,12,.6)', fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--r-cream)' }}>
            Campus B · night rounds
          </span>
        </Reveal>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'clamp(32px,5vw,64px)' }}>
          <p style={{ flex: '1 1 380px', margin: 0, fontSize: 15, lineHeight: 1.75, color: 'rgba(243,240,221,.62)' }}>
          ...
          <p style={{ flex: '1 1 320px', margin: 0, paddingLeft: 20, borderLeft: `2px solid ${ACCENT}`, fontFamily: "'Instrument Serif', Georgia, serif", fontStyle: 'italic', fontSize: 'clamp(19px,1.8vw,26px)', lineHeight: 1.4, color: 'var(--r-cream)' }}>
      ...
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', display: 'flex', flexDirection: 'column', gap: 'clamp(56px,8vh,96px)' }}>
        ...
        {processChapters.map((c, i) => (
          ...
          >
            <div style={{ flex: '1 1 400px', position: 'relative', aspectRatio: '4 / 3', borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(243,240,221,.08)' }}>
            ...
            <div style={{ flex: '1 1 360px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <span style={{ width: 24, height: 1, background: 'rgba(243,240,221,.3)' }} />
                <span style={{ fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>{c.label}</span>
              </div>
              ...
              {c.label === 'Strategy' ? (
                ...
                <p style={{ margin: 0, fontSize: 15, lineHeight: 1.75, color: 'rgba(243,240,221,.62)' }}>{c.body}</p>
              )}
              {c.tags && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                  {c.tags.map((tag) => (
                    <span key={tag} style={{ padding: '6px 14px', borderRadius: 999, border: '1px solid rgba(243,240,221,.14)', fontSize: 10, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(243,240,221,.62)' }}>
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      {/* Selected frames */}
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', background: '#1B1B18', display: 'flex', flexDirection: 'column', gap: 'clamp(28px,4vh,44px)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Reveal style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ position: 'relative', aspectRatio: '21 / 9', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(243,240,221,.08)' }}>
            ...
            <span style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', pointerEvents: 'none' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 999, background: 'rgba(12,12,12,.6)', fontSize: 11, letterSpacing: '.14em', textTransform: 'uppercase', color: ACCENT }}>
                Hover to preview ▶
              </span>
            </span>
          </div>
          <span style={{ fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Care that doesn&apos;t clock out — the film</span>
        </Reveal>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'clamp(24px,3vw,44px)' }}>
          {[
            ...
            <div key={pair.label} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.65, color: 'rgba(243,240,221,.62)' }}>{pair.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Impact */}
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', background: '#1B1B18', display: 'flex', flexDirection: 'column', gap: 'clamp(40px,5vh,60px)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <p style={{ margin: 0, maxWidth: '75ch', fontSize: 15, lineHeight: 1.75, color: 'rgba(243,240,221,.62)' }}>
      ...
      <section style={{ padding: 'clamp(90px,17vh,200px) clamp(20px,5vw,56px)', background: '#141412', display: 'flex', justifyContent: 'center' }}>
        <Reveal style={{ maxWidth: 820, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 30, textAlign: 'center' }}>
          <p style={{ margin: 0, fontFamily: "'Instrument Serif', Georgia, serif", fontStyle: 'italic', fontSize: 'clamp(28px,4.4vw,54px)', lineHeight: 1.3, color: 'var(--r-cream)' }}>
            &ldquo;They understood our story before they picked up a camera.&rdquo;
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
            <div style={{ width: 54, height: 54, borderRadius: '50%', background: '#2A2A22', border: '1px solid rgba(243,240,221,.1)' }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, textAlign: 'left' }}>
              <span style={{ fontSize: 15, fontWeight: 600 }}>Dr. Anaya Verma</span>
              <span style={{ fontSize: 12, color: 'rgba(243,240,221,.62)' }}>Chief of Communications · Aster Health</span>
            </div>
            <span style={{ width: 1, height: 34, background: 'rgba(243,240,221,.2)' }} />
      ...
      <section style={{ padding: 'clamp(60px,9vh,110px) clamp(20px,5vw,56px)', display: 'flex', flexDirection: 'column', gap: 'clamp(32px,4vh,48px)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 22 }}>
          {[
            ...
            <Link key={p.name} href="/#case-studies" style={{ display: 'flex', flexDirection: 'column', gap: 14, textDecoration: 'none', color: 'inherit' }}>
              <div style={{ position: 'relative', aspectRatio: '16 / 10', overflow: 'hidden', background: '#191915', border: '1px solid rgba(243,240,221,.08)' }}>
                <MediaSlot placeholder={`${p.name} — cover frame`} />
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 14 }}>
                <span style={{ fontSize: 18, fontWeight: 600 }}>{p.name}</span>
                <span style={{ fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>{p.category}</span>
              </div>
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6, color: 'rgba(243,240,221,.62)' }}>{p.copy}</p>
            </Link>
          ))}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'clamp(28px,4vw,56px)' }}>
          <Link href="/#case-studies" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 9, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Previous</span>
            <span style={{ fontSize: 13, letterSpacing: '.1em', textTransform: 'uppercase' }}>← Verma Dental</span>
          </Link>
          <Link href="/#case-studies" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 9, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(243,240,221,.45)' }}>Next</span>
